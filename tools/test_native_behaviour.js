...
(async () => {
  ...
  // Round 22 addendum 26: the press feedback has been
  // removed entirely. The user reported that every-other-tap
  // on a home feed video was being ignored, that Reels peeks
  // instead of pauses on tap, and that long-pressing a photo
  // to bring up the "Save" context menu did not register a
  // click on the menu items. All three came from the same
  // root cause: a dim class on a control-bar element told
  // Facebook's own pointer router "this was an interaction
  // in progress, not a tap", and the router treated the next
  // tap as the conclusion of an interaction that the dim
  // had already started. The browser's tap highlight is
  // suppressed above (transparent); the press effect was
  // the only visual feedback, and removing it gives a solid,
  // native feel with no race against the page's own input
  // handling. The pins below pin the new contract: the tap
  // highlight is still off, no class is added on touchstart,
  // and the touchstart listener that did add the class is
  // gone.
  console.log('\nA tap is a pure event - no class is added');
  {
    const ab = fs.readFileSync(KT('utils/AdBlocker.kt'), 'utf8');

    ok('the browser highlight is still off, as an app would have it',
       /-webkit-tap-highlight-color:transparent/.test(ab));
    ok('no __db_press class is defined in the native feel CSS',
       !/__db_press\s*\{/.test(ab));
    ok('no touchstart handler adds __db_press anywhere in the script',
       !/addEventListener\(\s*'touchstart'[\s\S]{0,400}__db_press/.test(ab));
    ok('and the held/timer/release IIFE is gone',
       !/var held = null, timer = null;/.test(ab));
  }
