...
(function() {
  if (window.__QBOOK_DOWNLOADER_LOADED__) return;
  window.__QBOOK_DOWNLOADER_LOADED__ = true;
  ...
  const SELECTORS = {
    mediaElements: [
      ...
      'div[role="dialog"] audio:not([hidden])',
  ...
  const getCurrentMediaElement = () => {
    ...
    return Array.from(
      document.querySelectorAll('video:not([hidden]), audio:not([hidden]), img[src*="fbcdn"]:not([width="16"]):not([hidden])')
    ).find(el => {
  ...
  const downloadMedia = (url) => {
    fetch(url)
      ...
      .then(blob => {
        if (window.QBookDownloadBridge && window.QBookDownloadBridge.downloadBase64File) {
          const reader = new FileReader();
          reader.onloadend = function() {
            if (reader.result) {
              window.QBookDownloadBridge.downloadBase64File(
                reader.result,
  ...
  const extractAndDownloadMedia = () => {
    ...
    // Find videos first
    const videoElement = container.querySelector("video:not([hidden]), audio:not([hidden])");
